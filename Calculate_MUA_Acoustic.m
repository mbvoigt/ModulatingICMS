%% Calculate MUA - Acoustic
%
% Subfunction for ModulatingICMS_main
%
% Calculates multi unit activity (MUA) responses of acoustically only 
% stimulated trials
%
function MUA = Calculate_MUA_Acoustic(cfg)

% Initialize variable
MUA = zeros(length(cfg.experiments),cfg.noChannels,30,cfg.sweepBefore+cfg.sweepAfter+1);
...
for eID = 1:length(cfg.experiments)
    ...
    fprintf('MUA | %s | Acoustic | \n',cfg.experiments{eID})
    expString = sprintf('MUA | %s | Acoustic | ',cfg.experiments{eID});
    mbv_log(expString,'file',cfg.logfile);
    mbv_log([expString 'Path: ' pth],'file',cfg.logfile);
    
    % Determine the available data files of files to load
    ...
    % Filter settings for multi unit activity
    ...
    MUA(eID,:,:,:) = dataFiltered(cfg.channelidx,:,:);
    clear dataFiltered
